...
class wp_ulike_widget extends WP_Widget {
    ...
    public function most_liked_posts(array $settings) {
        ...
        $request =  "SELECT posts.ID, posts.post_title, posts.post_content, posts.post_author, users.user_login, meta.meta_value
                    FROM
		...
		foreach ($posts as $post) {
			...
			$post_author = $post->user_login;
			$author_id = $post->post_author;
			$author_link = get_author_posts_url($author_id);
			$post_count = $post->meta_value;
			
			echo $settings['before_item'];
			echo "<div onclick='window.location.href="' . $permalink . '" title="' . $post_title.'"; return false' class='wp_ulike_imgcontainer' style='background-image:url(\"";
			...
			echo "<div>"; 
			echo $settings['show_count'] == '1' ? '<span class="hit-count"><img src="/wp-content/themes/ultra/inc/panels-lite/images/customized_icons/like.png" alt="Smiley face" width="16" height = "16">'.$post_count.'<img src="/wp-content/themes/ultra/inc/panels-lite/images/customized_icons/visited.png" alt="Smiley face" width="16" height = "16"></span>' : '';
			echo do_shortcode('[hit_count post='.$post->ID.']');
			echo "</div>"; 
	...
	/**
	 * Last Liked Posts Function
	 *
	 */

	public function last_liked_posts(array $settings) {
		global $wpdb;
        
        $request =  "SELECT DISTINCT posts.ID, posts.post_title, posts.post_content, posts.post_author, users.user_login, meta.meta_value
                    FROM
                    ".$wpdb->prefix."posts AS posts,
                    ".$wpdb->prefix."postmeta AS meta,
                    ".$wpdb->prefix."ulike AS likes,
                    ...
                    AND users.ID = posts.post_author
                    ".$this->period($settings['period'])."
                    ORDER BY likes.date_time DESC LIMIT ".$settings['numberOf']."
                    ";
    ...
    /** Get The Post Background
	 *
	 * @author       	Alimir
	 * @since           2.3
	 * @return			String
	 */		
	public function get_post_thumbnail_src($id,$sizeOf){
        $thumb = wp_get_attachment_image_src( get_post_thumbnail_id( $id ), 'large', true);
		if(!empty($thumb))
		return $thumb[0];
		else
		return ''.plugin_dir_url( __FILE__ ).'/img/no-thumbnail.png';
	}
	...
	public function widget( $args, $instance ) {
		...
		echo $this->last_liked_posts($settings);
		echo '</ul>';
