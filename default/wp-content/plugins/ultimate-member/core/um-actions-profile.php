<?php
	...
	***/
	...
	function um_profile_header( $args ) {
        ...
        $cats = array("行者", "方丈", "罗汉", "菩萨", "佛");
        
        $likes_on_all_posts = 0;
        $comments_on_all_posts = 0;
        $views_on_all_posts = 0;
        $favorites_on_all_posts = 0;
        $likes_on_all_comments = 0;
        
        $cs_user_id = um_profile_id();
        global $wpdb;
        
        $request = "SELECT count(*) FROM ".$wpdb->prefix."ulike INNER JOIN ".$wpdb->prefix."posts ON ".$wpdb->prefix."ulike.post_id = ".$wpdb->prefix."posts.ID WHERE ".$wpdb->prefix."ulike.status='like' AND ".$wpdb->prefix."posts.post_author = $cs_user_id";
		$posts = $wpdb->get_results($request);
        $likes_on_all_posts = $posts[0]->{'count(*)'};
        
                
        $request = "SELECT count(*) FROM ".$wpdb->prefix."comments INNER JOIN ".$wpdb->prefix."posts ON ".$wpdb->prefix."comments.comment_post_ID = ".$wpdb->prefix."posts.ID WHERE ".$wpdb->prefix."posts.post_author = $cs_user_id";
		$posts = $wpdb->get_results($request);
        $comments_on_all_posts = $posts[0]->{'count(*)'};
        
                
        $request = "SELECT SUM(meta_value) FROM ".$wpdb->prefix."postmeta INNER JOIN ".$wpdb->prefix."posts ON ".$wpdb->prefix."postmeta.post_id = ".$wpdb->prefix."posts.ID WHERE ".$wpdb->prefix."postmeta.meta_key='_post_views' AND ".$wpdb->prefix."posts.post_author = $cs_user_id";
		$posts = $wpdb->get_results($request);
        $views_on_all_posts = $posts[0]->{'SUM(meta_value)'};
        
                
        $request = "SELECT SUM(meta_value) FROM ".$wpdb->prefix."postmeta INNER JOIN ".$wpdb->prefix."posts ON ".$wpdb->prefix."postmeta.post_id = ".$wpdb->prefix."posts.ID WHERE ".$wpdb->prefix."postmeta.meta_key='wpfp_favorites' AND ".$wpdb->prefix."posts.post_author = $cs_user_id";
		$posts = $wpdb->get_results($request);
        $favorites_on_all_posts = $posts[0]->{'SUM(meta_value)'};
        
                
        $request = "SELECT SUM(meta_value) FROM ".$wpdb->prefix."commentmeta INNER JOIN ".$wpdb->prefix."comments ON ".$wpdb->prefix."commentmeta.comment_id = ".$wpdb->prefix."comments.comment_ID WHERE ".$wpdb->prefix."commentmeta.meta_key='_commentliked' AND ".$wpdb->prefix."comments.user_id = $cs_user_id";
		$posts = $wpdb->get_results($request);
        $likes_on_all_comments = $posts[0]->{'SUM(meta_value)'};
        ...
        $points = $likes_on_all_posts + $comments_on_all_posts + $views_on_all_posts + $favorites_on_all_posts + $likes_on_all_comments;
        $rem = 0;
        $level = $cats[0];
        if ($points < 100) {
            $rem = 100 - $points;
        ...
        <div class="um-header-custom">
            ...
            <div class="um-profile-photo" data-user_id="<?php echo um_profile_id(); ?>">
                    ...
                    <p class="um-header-profile-level"><?php echo $level; ?></p>
				</div>

            <div class="um-header-profile-name"><?php echo um_user('display_name'); ?></div>
            <div class="um-header-profile-bio" style="color: #aaaaaa; margin: 15px 0;">
            <?php
            
            if ($rem != -1) echo "(距离 $next_level 还差 $rem 点气)"; ?>
            
            </div>
            <div class="um-header-profile-bio"><?php if ( um_user('description') ) { echo um_user('description'); } ?></div>
        </div>
        </div>
